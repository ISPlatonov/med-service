...
def add_user():
    ...
    if name and email and pwd and request.method == "POST":
        ...
        msg = jsonify("Please provide name, email and pwd")
        msg.status_code = 404
        return msg
...
def login():
    """Function to authenticate a user in the MySQL database"""
    json = request.json
    email = json["email"]
    pwd = json["password"]
    if email and pwd:
        try:
            conn = mysql.connect()
            cursor = conn.cursor()
            cursor.execute("SELECT * FROM users WHERE user_email=%s AND user_password=%s", (email, pwd))
            row = cursor.fetchone()
            cursor.close()
            conn.close()
            if row:
